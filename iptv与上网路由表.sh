...
echo "1️⃣ 接口状态:"
ip -br addr show | grep -E "br-lan|phy1-sta0|pppoe-wan"
...
echo
echo "3️⃣ PPPoE 网关可达性:"
if ping -c 1 -W 1 "$PPPOE_GATEWAY" >/dev/null 2>&1; then
    echo "✅ PPPoE 网关 $PPPOE_GATEWAY 可达"
else
    echo "⚠ PPPoE 网关 $PPPOE_GATEWAY 不可达（可能是禁止 ping）"
fi
